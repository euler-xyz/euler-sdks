...
import { useEulerEarnDetail } from "../queries/sdkQueries";
import {
  formatBigInt,
  formatAPY,
  formatPercent,
...
export function EulerEarnDetailPage() {
  ...
  const chainIdValue = Array.isArray(chainIdParam)
    ? chainIdParam[0]
    : chainIdParam;
  ...
  useEffect(() => {
    if (Number.isFinite(chainId)) setChainId(chainId);
  }, [chainId, setChainId]);
  ...
  return (
    <>
      <Link href="/vaults" className="back-link">
        &larr; Back to vaults
      </Link>
