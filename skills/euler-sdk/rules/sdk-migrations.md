...
5. Contract wallets cannot sign the typed-data form — Aave, Morpho, and
   ...
   signature), and send `revocation` after the migration attempt whenever the
   authorization is active. Morpho omits `call` when authorization already
   stands but still returns the disable call as `revocation`. Use the
