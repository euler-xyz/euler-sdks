...
Use `mergePlans` to compose compatible intents into one EVC batch and `describeBatch` for previews/logging. `mergePlans` preserves every call, including repeated or opposing collateral/controller transitions; initial state, intervening calls, and status checks make automatic cancellation unsafe. It sums required approvals and clears previous approval resolutions. Combining batches changes transaction/status-check boundaries, so resolve approvals again and simulate the merged plan under the intended chain and execution context. It does not guarantee the same behavior as executing the input plans separately.
