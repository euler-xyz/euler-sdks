/**
 * EULER -> AAVE POSITION MIGRATION EXAMPLE
 *
 * This example prepares an Euler WETH/USDC position on a mainnet fork and
 * migrates it into Aave V3 variable debt without closing the user's borrowed
 * USDC exposure.
 *
 * OPERATION:
 *   1. Deposit WETH collateral and borrow USDC on Euler
 *   2. Sign Aave variable-debt delegation for the Euler SwapVerifier
 *   3. Withdraw Euler collateral to Swapper, supply it to Aave, borrow USDC
 *      from Aave, and repay Euler debt
 *
 * USAGE:
 *   1. Set FORK_RPC_URL in examples/.env
 *   2. Start Anvil: npm run anvil
 *   3. Run: npx tsx execution/euler-to-aave-position-migration-example.ts
 */
...
const MIGRATION_SUB_ACCOUNT_ID = 10;
const EULER_ACCOUNT = getSubAccountAddress(
	account.address,
	MIGRATION_SUB_ACCOUNT_ID,
);

async function eulerToAavePositionMigrationExample({
	walletClient,
	...
	console.log("\n=== Step 2: Sign Aave Variable-Debt Delegation ===");
	const eulerSource = {
	...
	console.log(
		signedAuthorization
			? "  Signed Aave variable-debt delegation for Euler SwapVerifier"
			: "  Aave variable-debt delegation already set",
	);

	console.log("\n=== Step 3: Migrate Euler Position to Aave ===");
...
initExample()
	.then(eulerToAavePositionMigrationExample)
	.catch((error) => {
