...
function convertOraclePrice(price: V3OraclePrice, errors: DataIssue[], path: string, entityId: Address): OraclePrice {
  const converted = {
    queryFailure: price.queryFailure,
    queryFailureReason: (price.queryFailureReason || "0x") as Hex,
    amountIn: parseBigIntField(price.amountIn, `${path}.amountIn`, entityId, errors),
    amountOutMid: parseBigIntField(price.amountOutMid, `${path}.amountOutMid`, entityId, errors),
    amountOutBid: parseBigIntField(price.amountOutBid, `${path}.amountOutBid`, entityId, errors),
    amountOutAsk: parseBigIntField(price.amountOutAsk, `${path}.amountOutAsk`, entityId, errors),
...
function convertCollaterals(
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const borrowLTV = parseRatio1e4(row.borrowLTV, `$.collaterals[${index}].borrowLTV`, collateralAddress, errors);
    ...
    const isRemovedCollateral = borrowLTV === 0 && liquidationLTV === 0 && targetTimestamp < vaultTimestamp;
    ...
    const collateral: EVaultCollateral = {
      ...
      oraclePriceRaw: row.oraclePriceRaw
        ? convertOraclePrice(row.oraclePriceRaw, errors, `$.collaterals[${collaterals.length}].oraclePriceRaw`, collateralAddress)
        : {
            queryFailure: true,
    ...
    if (!row.oraclePriceRaw) {
      errors.push({
        code: "DEFAULT_APPLIED",
        severity: "warning",
        message: "Missing collateral oraclePriceRaw; default zero-price placeholder applied.",
...
function convertVault(detail: V3VaultDetail, collateralRows: V3CollateralRow[], errors: DataIssue[]): IEVault {
  ...
  const fees: EVaultFees = {
    ...
    accumulatedFeesShares: parseBigIntField(detail.fees.accumulatedFeesShares, "$.fees.accumulatedFeesShares", entityId, errors),
    accumulatedFeesAssets: parseBigIntField(detail.fees.accumulatedFeesAssets, "$.fees.accumulatedFeesAssets", entityId, errors),
  ...
  const caps: EVaultCaps = {
    supplyCap: parseBigIntField(detail.caps.supplyCap, "$.caps.supplyCap", entityId, errors),
    borrowCap: parseBigIntField(detail.caps.borrowCap, "$.caps.borrowCap", entityId, errors),
  };
  ...
  return {
    ...
    totalCash: parseBigIntField(detail.totalCash, "$.totalCash", entityId, errors),
    totalBorrowed: parseBigIntField(detail.totalBorrowed, "$.totalBorrowed", entityId, errors),
    ...
    oraclePriceRaw: convertOraclePrice(detail.oraclePriceRaw, errors, "$.oraclePriceRaw", entityId),
...
export class EVaultV3Adapter implements IEVaultAdapter {
  ...
  queryV3VaultDetail = async (endpoint: string, chainId: number, vault: Address): Promise<V3Envelope<V3VaultDetail>> => {
    ...
    if (!response.ok) throw new Error(`eVaultV3 detail ${response.status} ${response.statusText}`);
  ...
  queryV3VaultCollaterals = async (
    ...
    if (!response.ok) throw new Error(`eVaultV3 collaterals ${response.status} ${response.statusText}`);
  ...
  async fetchVaults(chainId: number, vaults: Address[]): Promise<ServiceResult<(IEVault | undefined)[]>> {
    const results: Array<{ result: IEVault | undefined; errors: DataIssue[] }> = await Promise.all(
      vaults.map(async (vault, index) => {
        ...
        try {
          const [detailResponse, collateralsResponse] = await Promise.all([
            ...
            this.queryV3VaultCollaterals(this.config.endpoint, chainId, vault),
          ]);
          ...
          const converted = convertVault(detail, collateralsResponse.data ?? [], errors);
          return {
            result: converted,
