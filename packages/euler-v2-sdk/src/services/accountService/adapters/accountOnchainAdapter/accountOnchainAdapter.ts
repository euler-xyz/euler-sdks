...
export class AccountOnchainAdapter implements IAccountAdapter {
	...
	async fetchAccount(
		...
		const accountVaults = await this.positionsAdapter.fetchAccountVaults(
			chainId,
			address,
		);
		const subAccountAddresses = [
			...new Set(
				Object.keys(accountVaults).map((subAccountAddress) =>
					getAddress(subAccountAddress),
				),
			),
		];
		...
		const subAccountsArray = await Promise.all(
			subAccountAddresses.map(async (subAccountAddress) => {
				const vaults = [
					...new Set([
		...
		const validSubs = subAccountsArray
			...
			.filter(
				(
					subAccount,
				): subAccount is ISubAccount & {
					isLockdownMode: boolean;
					isPermitDisabledMode: boolean;
				} => subAccount !== undefined,
			);

		const subAccounts = validSubs.reduce<Record<Address, ISubAccount>>(
			(acc, sa) => {
				const {
					isLockdownMode: _lm,
	...
	> {
		...
		if (this.plugins.length > 0) {
			vaultAccountInfos = await this.fetchVaultAccountInfosWithPlugins(
				chainId,
				provider,
				accountLensAddress,
				subAccount,
	...
	private async fetchVaultAccountInfosWithPlugins(
						...
						(result) =>
							new EVault(
								convertVaultInfoFullToIEVault(
									result as unknown as VaultInfoFull,
									chainId,
									[],
								),
							),
		...
		const prepend =
			eVaults.length > 0
				? await this.collectReadPrepend(chainId, eVaults)
				: null;
