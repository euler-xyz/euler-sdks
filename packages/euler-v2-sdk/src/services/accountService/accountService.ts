...
> {
	...
	): Promise<ServiceResult<Account<TVaultEntity>>>;
	fetchSubAccount(
		chainId: number,
		subAccount: Address,
		vaults?: Address[],
...
> implements IAccountService<TVaultEntity>
{
	...
	async fetchSubAccount(
		...
		const resolvedOptions = this.resolveFetchOptions(options);
		const fetched = await this.adapter.fetchSubAccount(
			chainId,
			subAccount,
			vaults,
		);
		...
		if (resolvedOptions.populateVaults === false) {
			return {
				result: new SubAccount(sa) as SubAccount<TVaultEntity>,
				errors: compressDataIssues(errors),
			};
		}

		const addresses = vaults?.length
			? vaults
			: collectVaultAddressesFromSubAccountPositionsAndLiquidity(sa);
		...
		errors.push(
			...(await tempAccount.populateVaults(
				this.vaultMetaService,
				this.buildVaultFetchOptions(resolvedOptions),
			)),
		);
		return {
	...
	private buildVaultFetchOptions(
		options?: AccountFetchOptions,
	): VaultFetchOptions | undefined {
		if (!options?.vaultFetchOptions && options?.populateAll !== true)
			return undefined;
