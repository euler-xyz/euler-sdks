...
export class IntrinsicApyService implements IIntrinsicApyService {
  ...
  constructor(config?: IntrinsicApyServiceConfig, buildQuery?: BuildQueryFn) {
    ...
    this.stablewatchPoolsUrl = config?.stablewatchPoolsUrl ?? DEFAULT_STABLEWATCH_POOLS_URL;
    this.stablewatchSourceUrl = config?.stablewatchSourceUrl ?? DEFAULT_STABLEWATCH_SOURCE_URL;
  ...
  private async fetchStablewatch(
    ...
    const response = await this.queryStablewatchPools(this.stablewatchPoolsUrl)
    ...
    for (const pool of pools) {
      ...
      for (const [rawChainName, addresses] of Object.entries(pool.token.chains)) {
        ...
        for (const address of addresses) {
          ...
          lookup.set(buildStablewatchLookupKey(normalizedChain, address), Math.max(0, apy));
        }
      }
    }
    ...
    for (const source of stablewatchSources) {
      const apy = lookup.get(buildStablewatchLookupKey(chainName, source.address));
