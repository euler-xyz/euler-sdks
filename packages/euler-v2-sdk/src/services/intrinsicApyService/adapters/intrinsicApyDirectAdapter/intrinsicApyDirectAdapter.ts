...
export class IntrinsicApyDirectAdapter implements IIntrinsicApyAdapter {
	...
	constructor(
		config?: IntrinsicApyDirectAdapterConfig,
	...
	async fetchChainIntrinsicApys(
		...
		const requestedAddresses = assetAddresses
			? new Set(assetAddresses.map((address) => address.toLowerCase()))
			: undefined;
