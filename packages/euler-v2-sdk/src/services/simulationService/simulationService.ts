	...
	implements ISimulationService<TVaultEntity>
{
	...
	async simulateTransactionPlan(
		...
		if (useStateOverrides) {
			effectiveStateOverrides = await this.deriveStateOverrides(
				chainId,
				owner,
				transactionPlan,
			);
		...
		const failedBatchItems = (
			await Promise.all(
				rawBatchResults.map(async (itemResult, index) => {
					...
					const decodedError = await decodeSmartContractErrors(
						itemResult.result,
					);
					const decodedItem =
		...
		if (vaultFetchOptions?.populateIntrinsicApy && this.intrinsicApyService) {
			await this.intrinsicApyService.populateIntrinsicApy(
				simulatedVaults as any,
			);
		}
		...
		for (const [subAccount, evcInfo] of evcInfos.entries()) {
			...
			const built = this.accountAdapter.buildSubAccount(
				evcInfo,
				vaultInfos,
				[],
			);
	...
	private async buildSimulationBatch(
		...
		const vaultTypes = await this.vaultMetaService.fetchVaultTypes(
			chainId,
			vaultCandidatesList,
		);
