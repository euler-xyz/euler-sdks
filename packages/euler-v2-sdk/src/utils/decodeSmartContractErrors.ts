...
import {
	EULER_ERROR_SELECTOR_TO_SIGNATURE,
	EULER_ERROR_SIGNATURES,
} from "./eulerErrorSelectors.js";

const FOURBYTE_LOOKUP_URL =
	"https://api.4byte.sourcify.dev/signature-database/v1/lookup?function=";
const HEX_PATTERN = /0x[0-9a-fA-F]{8,}/g;
const SELECTOR_WITH_PAYLOAD_PATTERN =
	/0x([0-9a-fA-F]{8})\s*[:|-]?\s*([0-9a-fA-F]{64,})/g;
...
const normalizeHex = (value: string): Hex | null => {
	...
	return (
		clean.length % 2 === 0 ? clean : clean.slice(0, clean.length - 1)
	) as Hex;
};
...
const fetchSignaturesBySelector = (
	selector: string,
	fetchTimeout: number,
): Promise<string[]> => {
	...
	const request = (async () => {
		...
		try {
			const response = await fetch(
				`${FOURBYTE_LOOKUP_URL}${normalizedSelector}`,
...
const addKnownErrorsFromCandidate = (
	candidate: string,
	output: Set<string>,
) => {
...
export async function decodeSmartContractErrors(
	...
	const fetchTimeout =
		typeof options.fetchTimeout === "number" && options.fetchTimeout >= 0
	...
	const addKnownErrorsFromCandidateToResults = (candidate: string) => {
		...
		signatures.forEach((signature) =>
			addResult(signature, selectorBySignature.get(signature) ?? null, []),
		);
	};
	...
	const processHexValue = async (hexValue: Hex): Promise<void> => {
		...
		const builtinSignature =
			BUILTIN_SELECTOR_TO_SIGNATURE[selector.toLowerCase()];
	...
	return results.filter((entry) => {
		...
		return !entriesWithParams.has(
			JSON.stringify([entry.signature, entry.selector]),
		);
	});
}
