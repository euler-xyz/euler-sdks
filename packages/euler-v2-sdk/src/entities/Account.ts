...
export interface AccountLiquidityCollateral<
...
> {
	...
	/** Collateral value in USD. `collateral deposit amount * marketPriceUsd`. */
	valueUsd?: number;
}
...
export class AccountLiquidity<TVaultEntity extends IHasVaultAddress = never>
	implements IAccountLiquidity<TVaultEntity>
{
	vaultAddress: Address;
	vault?: TVaultEntity;
	unitOfAccount: Address;
	...
	get collateralLiquidationPrices(): Record<Address, bigint> {
		return computeCollateralLiquidationPrices(
			this as unknown as IAccountLiquidity<IHasVaultAddress>,
		);
	}

	/** Borrow liquidation price multiplier (WAD). `> 1` = safe margin. */
	get borrowLiquidationPrice(): bigint | undefined {
		return computeBorrowLiquidationPrice(
			this as unknown as IAccountLiquidity<IHasVaultAddress>,
		);
	}
}
...
export class SubAccount<TVaultEntity extends IHasVaultAddress = never>
	implements ISubAccount<TVaultEntity>
{
	timestamp: number;
	account: Address;
	owner: Address;
...
export type SubAccountsMap<TVaultEntity extends IHasVaultAddress = never> =
	Partial<Record<Address, SubAccount<TVaultEntity>>>;
...
export class Account<TVaultEntity extends IHasVaultAddress = never>
	implements IAccount<TVaultEntity>
{
	chainId: number;
	owner: Address;
	isLockdownMode: boolean;
	...
	getPosition(
		account: Address,
		vault: Address,
	): AccountPosition<TVaultEntity> | undefined {
		const subAccount = this.getSubAccount(getAddress(account));
		return subAccount?.positions.find((p) =>
			isAddressEqual(p.vaultAddress, getAddress(vault)),
		);
	}
	...
	isCollateralEnabled(subAccountAddress: Address, vault: Address): boolean {
		...
		return subAccount.enabledCollaterals.some((coll) =>
			isAddressEqual(coll, getAddress(vault)),
		);
	}
	...
	isControllerEnabled(subAccountAddress: Address, vault: Address): boolean {
		const subAccount = this.getSubAccount(getAddress(subAccountAddress));
		if (!subAccount) return false;
		return subAccount.enabledControllers.some((ctrl) =>
			isAddressEqual(ctrl, getAddress(vault)),
		);
	}
	...
	async populateVaults<TResolved extends IHasVaultAddress>(
		...
		const fetched = await vaultMetaService.fetchVaults(
			this.chainId,
			addresses,
			options,
		);
	...
	mapVaultsToPositions<TResolved extends IHasVaultAddress>(
		...
		const resolve = (addr: Address): TResolved | undefined =>
			byAddress.get(getAddress(addr));
		for (const sa of Object.values(this.subAccounts ?? {})) {
			if (!sa) continue;
			for (const p of sa.positions) {
				const entity = resolve(p.vaultAddress);
				if (entity !== undefined)
					(p as unknown as AccountPosition<TResolved>).vault = entity;
				if (p.liquidity) {
	...
	async populateMarketPrices(
		...
		if (!this.populated.vaults) {
			throw new Error(
				"populateMarketPrices requires populateVaults to be called first.",
			);
		}
		...
		for (const sa of Object.values(this.subAccounts ?? {})) {
			...
			for (const p of sa.positions) {
				...
				// Populate liquidity values from actual borrow/deposit amounts.
				if (p.liquidity) {
					p.liquidity.liabilityValueUsd = p.borrowedValueUsd;
					let totalCollateralValueUsd: number | undefined = 0;
					for (const c of p.liquidity.collaterals) {
						...
						if (collateralAssets === 0n) {
							...
							c.valueUsd = tokenAmountToUsdValue(
								collateralAssets,
						...
						} else if (totalCollateralValueUsd != null) {
	...
	async populateUserRewards(
		...
		try {
			this.userRewards = await rewardsService.fetchUserRewards(
				this.chainId,
				this.owner,
			);
