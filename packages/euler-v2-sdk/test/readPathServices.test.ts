...
test("account service fetches and populates real account fixtures through vault, price, and reward services", async () => {
	...
	const borrowWithLiquidity = Object.values(fetched.result.subAccounts)
		.filter(Boolean)
		.flatMap((subAccount) => subAccount!.positions)
		.find(
			(position) =>
...
test("evault service setters and empty branches are exercised", async () => {
	...
	await assert.rejects(
		() => directPerspectiveService.fetchVerifiedVaultAddresses(1, ["evkFactoryPerspective"]),
