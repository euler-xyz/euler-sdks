...
import { getCollateralizedEVaultFixture, getPlainEVaultFixture } from "./helpers/readCorpus.ts";
	...
	try {
		...
		Map.prototype.get = function patchedGet(key) {
			...
			if (
				...
				return { ...(value as object), promise: Promise.resolve("other") } as any;
			}
			return value;
		};
...
test("deployment, provider, abi, tokenlist, intrinsic apy, wallet, and labels services cover their read flows", async () => {
	...
	assert.deepEqual(deploymentService.getDeploymentChainIds().sort((a, b) => a - b), [1, 8453]);
	assert.equal(
	...
	assert.throws(() => deploymentService.getDeployment(999), /Deployment not found/);

	const providerService = new ProviderService({
		1: "https://ethereum-rpc.publicnode.com",
		8453: "https://base-rpc.publicnode.com",
	});
	assert.deepEqual(providerService.getSupportedChainIds().sort((a, b) => a - b), [1, 8453]);
	assert.equal(providerService.getProvider(1).chain?.id, 1);
	assert.throws(() => providerService.getProvider(10), /No provider configured/);
	assert.throws(
	...
	assert.equal(
		tokenlistService.getToken(1, "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48")?.symbol,
		"USDC",
	);
	assert.throws(() => tokenlistService.getToken(8453, zeroAddress), /not loaded/);

	const intrinsicAdapterCalls: Array<{ chainId: number; assetAddresses?: Address[] }> = [];
	...
	await intrinsicApyService.populateIntrinsicApy([
		plainVault,
		collateralVault,
		new EVault({ ...getPlainEVaultFixture(), address: getAddress("0x00000000000000000000000000000000000000aa") }),
	]);
	...
	assert.deepEqual(await intrinsicApyService.fetchChainIntrinsicApys(1), new Map());
	...
	assert.equal(await intrinsicApyService.fetchIntrinsicApy(1, plainVault.asset.address), undefined);
	await intrinsicApyService.populateIntrinsicApy([]);
	await intrinsicApyService.populateIntrinsicApy([
	...
	walletAdapter.setQueryAllowance(async (_provider, asset, _owner, spender) => {
		if (asset === collateralVault.asset.address && spender === plainVault.address) {
			throw new Error("allowance");
		}
		return 456n;
	});
	walletAdapter.setQueryPermit2Allowance(async (_provider, _permit2, _owner, asset) => {
	...
	walletService.setAdapter({
		async fetchWallet() {
			return {
				...
				errors: [{ code: "X", severity: "warning", message: "x", paths: ["$"] }],
			};
		},
	});
	...
	assert.equal(
		wallet.result.getAllowances(plainVault.asset.address, plainVault.address)?.assetForVaultInPermit2,
		3n,
	);
	assert.equal(wallet.result.getAsset(collateralVault.asset.address), undefined);
	assert.deepEqual(new Wallet(wallet.result).assets, wallet.result.assets);

	const labelsAdapter = new EulerLabelsURLAdapter(
		{
			getEulerLabelsVaultsUrl: (chainId) => `https://labels/${chainId}/vaults`,
			getEulerLabelsEntitiesUrl: (chainId) => `https://labels/${chainId}/entities`,
			getEulerLabelsProductsUrl: (chainId) => `https://labels/${chainId}/products`,
			getEulerLabelsPointsUrl: (chainId) => `https://labels/${chainId}/points`,
			getEulerLabelsEarnVaultsUrl: (chainId) => `https://labels/${chainId}/earn-vaults`,
			getEulerLabelsLogoUrl: (filename) => `https://cdn/${filename}`,
		},
		buildQuery,
	);
	labelsAdapter.setQueryEulerLabelsVaults(async (url) => {
		assert.equal(url, "https://labels/1/vaults");
		return {
			[plainVault.address]: {
				name: "Prime Vault",
				entity: "euler",
	...
	labelsAdapter.setQueryEulerLabelsPoints(async (url) => {
		...
		return [{ name: "Points", collateralVaults: [plainVault.address], logo: "points.svg" }] as any;
	});
	assert.equal(Object.keys(await labelsAdapter.fetchEulerLabelsVaults(1)).length, 1);
	assert.equal(Object.keys(await labelsAdapter.fetchEulerLabelsEntities(1)).length, 1);
	assert.equal(Object.keys(await labelsAdapter.fetchEulerLabelsProducts(1)).length, 1);
	assert.equal((await labelsAdapter.fetchEulerLabelsPoints(1)).length, 1);

	const labelsService = new EulerLabelsService(labelsAdapter, (filename) => `https://cdn/${filename}`);
	await labelsService.populateLabels([plainVault, collateralVault]);
	await labelsService.populateLabels([]);
	assert.equal(plainVault.eulerLabel?.vault.name, "Prime Vault");
	assert.equal(plainVault.eulerLabel?.entities[0]?.logo, "https://cdn/euler.svg");
	assert.equal(plainVault.eulerLabel?.products[0]?.logo, "https://cdn/product.svg");
	assert.equal(plainVault.eulerLabel?.points[0]?.logo, "https://cdn/points.svg");
	...
	labelsService.setAdapter({
		...
		async fetchEulerLabelsPoints() {
			return [
				...
				{ name: "NoLogoCollateral", collateralVaults: ["0x00000000000000000000000000000000000000ac"] },
			] as any;
		},
	});
	...
	assert.deepEqual(bareVault.eulerLabel?.entities, []);
	assert.deepEqual(bareVault.eulerLabel?.products, []);
	assert.deepEqual(bareVault.eulerLabel?.points, []);
	...
	try {
		...
		globalThis.fetch = (async (input: RequestInfo | URL) => {
			...
			if (url.includes("abi-ok")) {
				return {
					ok: true,
					json: async () => [{ type: "function", name: "x", inputs: [], outputs: [] }],
				} as Response;
			}
			...
			if (url.includes("labels-vaults-ok")) {
				return {
					ok: true,
					json: async () => ({ [zeroAddress]: { name: "Vault" } }),
				} as Response;
			}
			...
			if (url.includes("labels-products-ok")) {
				return {
					ok: true,
					json: async () => ({ product: { name: "Product", vaults: [zeroAddress] } }),
				} as Response;
			}
			if (url.includes("labels-points-ok")) {
				return {
					ok: true,
					json: async () => [{ name: "Point", collateralVaults: [zeroAddress] }],
				} as Response;
			}
		...
		await assert.rejects(() => DeploymentService.queryDeployments(""), /Deployments URL is required/);
		...
		assert.deepEqual(
			await abiService.queryABI("https://abi-ok"),
			[{ type: "function", name: "x", inputs: [], outputs: [] }],
		);

		const tokenlistService = new TokenlistService({ getTokenListUrl: () => "" });
		tokenlistService.setQueryTokenList(tokenlistService.queryTokenList);
		assert.deepEqual(await tokenlistService.queryTokenList("https://tokens-ok"), [
			{ chainId: 1, address: "0x00000000000000000000000000000000000000aa" },
		] as any);
		await assert.rejects(
		...
		const labelsAdapter = new EulerLabelsURLAdapter({
			getEulerLabelsVaultsUrl: () => "https://labels-vaults-ok",
			...
			getEulerLabelsEarnVaultsUrl: () => "https://labels-earn",
		...
		assert.equal(Object.keys(await labelsAdapter.fetchEulerLabelsProducts(1)).length, 1);
		assert.equal((await labelsAdapter.fetchEulerLabelsPoints(1)).length, 1);
		await assert.rejects(() => labelsAdapter.queryEulerLabelsVaults("https://labels-vaults"), /Failed to fetch Euler labels vaults/);
		await assert.rejects(() => labelsAdapter.queryEulerLabelsEntities("https://labels-entities"), /Failed to fetch Euler labels entities/);
		await assert.rejects(() => labelsAdapter.queryEulerLabelsProducts("https://labels-products"), /Failed to fetch Euler labels products/);
		await assert.rejects(() => labelsAdapter.queryEulerLabelsPoints("https://labels-points"), /Failed to fetch Euler labels points/);
	} finally {
		DeploymentService.setQueryDeployments(originalQueryDeployments);
